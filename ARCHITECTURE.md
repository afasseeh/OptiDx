...
9. The optimization wizard creates an `optimization_runs` record through `/api/pathways/optimize`, stores the selected `run_mode`, and Laravel dispatches either a detached Artisan worker for `light` runs or a queued `ExecuteOptimizationRun` job for `extensive` runs. Light runs are therefore able to start in local development without a live queue worker, while extensive runs remain in the queue after the browser navigates away and are resumed from the persisted run record. Both the detached launcher and the Python bridge force Symfony Process onto a project-local writable temp directory so Windows installations that resolve `C:\Windows` as the default temp location do not fail before the child process starts. The detached launcher must also resolve a CLI-capable PHP binary explicitly, because web requests on Windows can run under `php-cgi.exe`, which is not a safe background Artisan runtime.
